...
color_names, color_xyY = load_babel(output_coords="xyY")# load the names and XYZ coordinates of the 24 default Babel colors
...
champion_pops_array = np.empty((len(n_junc_loop), len(Y_values), len(color_xyY)), dtype=object)
...
# Need this __main__ construction because otherwise the parallel running of the different islands (n_trials) may throw an error
    if calc:
        ...
        for i1, n_junctions in enumerate(n_junc_loop):
            ...
            for j1, Y in enumerate(Y_values):
                ...
                save_loc = "results/champion_eff_Y_" + color_suff[j1] + '_' + type + str(n_peaks) + '_' + str(
                ...
                color_XYZ = np.array([convert_color(xyYColor(*x), XYZColor).get_value_tuple() for x in color_xyY])
                ...
                color_names = [x + "_" + color_suff[j1] for x in color_names_pre[poss_colors]]
                ...
                np.savetxt("results/champion_eff_Y_" + color_suff[j1] + '_' + type + str(n_peaks) + '_' + str(
                    n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base)+'_spd.txt', champion_effs)
                np.savetxt("results/champion_pop_Y_" + color_suff[j1] + '_' + type + str(n_peaks) + '_' + str(
                    n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base) +'_spd.txt', champion_pops)
                np.save("results/final_pop_Y_" + color_suff[j1] + '_' + type + str(n_peaks) + '_' + str(
        ...
        champion_effs_array = np.load("results/champion_effs_array_spd.npy", allow_pickle=True)
        champion_pops_array = np.load("results/champion_pops_array_spd.npy", allow_pickle=True)
    ...
    for i1, n_junc in enumerate([1,2,3,4]):
        ...
        ax.plot(Y_plot, plot_data,linestyle=linestyle[i1], marker=shapes[i1],
                 mfc='none')

        ax.plot(0,1, 'k', linestyle=linestyle[i1], marker=shapes[i1], mfc='none', label=str(n_junc))
    ...
    ax.set_xlim(-0.03, 1)
    ax.set_ylim(23, 56)
    ax.legend(title="Junctions:", bbox_to_anchor=(1.22, 0.8),
              frameon=False)
    plt.tight_layout()
    ax1.legend(title="Colour:", bbox_to_anchor=(1.25, 0.4),
               frameon=False)
    plt.tight_layout()
    ...
    fig, ax = plt.subplots()
    for i1, n_junc in enumerate([1,2,3,4]):
        plot_data = np.insert(champion_effs_array[n_junc-1], 0, black_cell_eff[n_junc-1], axis=0)
        ax.plot(Y_plot, plot_data/np.nanmax(plot_data),
                linestyle=linestyle[i1], marker=shapes[i1],
                mfc='none')

        ax.plot(0, 2, 'k', linestyle=linestyle[i1], marker=shapes[i1], mfc='none', label=str(n_junc))
    ...
    for i1, n_junc in enumerate([1, 2, 3, 4]):
        ...
        for l1 in range(len(pops)):
            if pops[l1] is None:
                ...
                IVs[l1] = getIVtandem(pops[l1][-n_junc:][::-1], photon_flux_cell[1], photon_flux_cell[0], interval)


        pops = np.vstack(pops)
        pops = np.reshape(pops, (20,4, n_params))
        IVs = np.reshape(IVs, (20,4,2))
        axes[0].plot(Y_values, IVs[:,:,1],
                linestyle=linestyle[i1], marker=shapes[i1],
                mfc='none')

        axes[0].plot(0, 1, 'k', linestyle=linestyle[i1], marker=shapes[i1], mfc='none', label=str(n_junc))

        axes[1].plot(Y_values, IVs[:,:,0],
                linestyle=linestyle[i1], marker=shapes[i1],
                mfc='none')

    for j1 in range(len(color_xyY)):
        axes[1].plot(-100, -100, '-o', mfc='none', color=pal[j1], label=color_labels[j1])
    ...
    # ax.set_ylim(0.5, 1)
    axes[0].legend(title="Junctions:", bbox_to_anchor=(1.22, 0.8),
              frameon=False)
    plt.tight_layout()
    axes[1].legend(title="Colour:", bbox_to_anchor=(1.25, 0.4),
               frameon=False)
    plt.tight_layout()
