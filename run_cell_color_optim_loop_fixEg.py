from color_cell_optimization import load_babel, multiple_color_cells, \
    cell_optimization, getIVmax, gen_spectrum_ndip, getPmax
import numpy as np
...
Y_cols = Y[:18]
col_names = xr.DataArray(data=color_names[:18], dims=['Y'],
                         coords={'Y': Y_cols})
...
for n_junctions in n_junc_loop:
    ...
    if n_junctions > 0:
        ...
        save_loc = save_path + "/champion_pop_{}juncs_{}spec_Si.txt".format(n_junctions, light_source_name)
...
# Need this __main__ construction because otherwise the parallel running of the different islands (n_trials) may throw an error
    ...
    for n_peaks in n_peak_loop:
        for i1, n_junctions in enumerate(n_junc_loop):
            ...
            if n_junctions > 0:
                Eg_guess = np.loadtxt(save_path + "/champion_pop_{}juncs_{}spec_Si.txt".format(n_junctions, light_source_name),
                                      ndmin=1)
            ...
            for fixed_height in fixed_height_loop:
                ...
                save_name = "results/champion_eff_" + type + str(n_peaks) + '_' + str(
                    n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base)+'_Si_spd.txt'
                ...
                if not path.exists(save_name):
                    ...
                    result = multiple_color_cells(color_XYZ, color_names, photon_flux_cell,
                                              ...
                                              max_trials_col=max_trials_col, base=base, max_height=max_height,
                                                Eg_black=Eg_guess, fixed_bandgaps=fixed_bandgaps,
                                              plot=False)
                    ...
                    np.savetxt("results/champion_eff_" + type + str(n_peaks) + '_' + str(
                        n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base)+'_Si_spd.txt', champion_effs)
                    np.savetxt("results/champion_pop_" + type + str(n_peaks) + '_' + str(
                        n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base) +'_Si_spd.txt', champion_pops)
                    np.save("results/final_pop_" + type + str(n_peaks) + '_' + str(
                        n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base)+'_3J_spd.npy', final_populations)
    ...
    unconst_1j = np.loadtxt("results/champion_eff_" + type + str(2) + '_' + str(
                        1) + '_' + 'True' + str(max_height) + '_' + str(base) + '_spd.txt')
    unconst_2j = np.loadtxt("results/champion_eff_" + type + str(2) + '_' + str(
                        2) + '_' + 'True' + str(max_height) + '_' + str(base) + '_spd.txt')
    unconst_3j = np.loadtxt("results/champion_eff_" + type + str(2) + '_' + str(
        3) + '_' + 'True' + str(max_height) + '_' + str(base) + '_spd.txt')
    ...
    #
    # eff_xr_col = xr.DataArray(data=max_effs[:, :18].flatten(), dims=['color'],
    #                           coords={'color': Y_cols})
    #
    # eff_xr_col = eff_xr_col.sortby('color', ascending=False)
    # eff_xr_col = eff_xr_col.assign_coords(color=col_names.data)
    #
    # black_cell_eff = getPmax(fixed_bandgaps, photon_flux_cell[1], wl_cell, interval) / 10
    #
    # eff_xr_bw = xr.DataArray(data=max_effs[:,18:].flatten().tolist() + [black_cell_eff], dims=['color'],
    #                          coords={'color': color_names[18:].tolist() + ['Black']})
    #
    # eff_xr = xr.concat([eff_xr_col, eff_xr_bw], dim='color')
    #
    # pop_xr_col = xr.DataArray(data=champion_pops[:18,:], dims=['color', 'peak'],
    #                         coords={'color': Y_cols})
    #
    # pop_xr_col = pop_xr_col.sortby('color', ascending=False)
    # pop_xr_col = pop_xr_col.assign_coords(color=col_names.data)
    ...
    # ax2 = ax.twinx()
    #
    # # ax.plot(color_names, unconst[2], marker=shapes[1], mfc='none',
    # #         color='k', alpha=0.5, linestyle='none', markersize=5)
    #
    ...
    #     _, Is = getIVmax(fixed_bandgaps, (1-spec)*photon_flux_cell[1], wl_cell, interval)
    #     J1_c[k1] = Is[0]
    #     J2_c[k1] = Is[1]
    #     J3_c[k1] = Is[2]
    #
    ...
    # J2_c[-1] = Is[1]
    # J3_c[-1] = Is[2]
    # ax2.plot(eff_xr.color, J1_c/10, label="InGaP", color=pal[0], marker=shapes[1], alpha=0.5)
    # ax2.plot(eff_xr.color, J2_c/10, label="GaAs", color=pal[1], marker=shapes[2], alpha=0.5)
    # ax2.plot(eff_xr.color, J3_c/10, label="Ge", color=pal[2], marker=shapes[3], alpha=0.5)
    #
    # ax.plot(eff_xr.color.data, eff_xr, marker=shapes[0], linestyle='none',
    #         markersize=4, alpha=0.5, color='k')
    # ax.xaxis.set_ticks(ax.get_xticks())
    # ax.set_xticklabels(eff_xr.color.data, rotation=45, ha='right', rotation_mode='anchor')
    # ax.set_ylabel("Efficiency (%)")
    ...
    fig, ax = plt.subplots(1)
    unconst_1j_Eg = np.loadtxt("results/champion_pop_" + type + str(2) + '_' + str(
        1) + '_' + 'True' + str(max_height) + '_' + str(base) + '_spd.txt')[:,-1:]
    unconst_2j_Eg = np.loadtxt("results/champion_pop_" + type + str(2) + '_' + str(
        2) + '_' + 'True' + str(max_height) + '_' + str(base) + '_spd.txt')[:,-2:]
    unconst_3j_Eg = np.loadtxt("results/champion_pop_" + type + str(2) + '_' + str(
        3) + '_' + 'True' + str(max_height) + '_' + str(base) + '_spd.txt')[:,-3:]
    unconst = [unconst_1j_Eg, unconst_2j_Eg, unconst_3j_Eg]

    for i1 in n_junc_loop:
        ax.plot(color_names, np.vstack(best_Eg[i1]), marker=shapes[i1], linestyle='none',
                markersize=4, alpha=0.5, label=str(i1 + 1) + " junctions")

        # ax.plot(color_names, unconst[i1], marker=shapes[i1], mfc='none',
        #         color='k', alpha=0.5, linestyle='none', markersize=5)

    ax.legend()

    ax.xaxis.set_ticks(ax.get_xticks())
    ax.set_xticklabels(color_names.tolist(), rotation=45, ha='right', rotation_mode='anchor')
    ax.set_ylabel("Efficiency (%)")
