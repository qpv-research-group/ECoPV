        ...
        for pop_size in pop_sizes:
            for n_iters in n_i:
                ...
                for j1 in range(n_trials):
                    ...
                    internal_run = single_colour(plot_pareto=False)

                    eff_result_par = Parallel(n_jobs=-1)(delayed(internal_run.run)
                                                         (color_XYZ[i1], color_names[i1],
                                                          col_thresh, photon_flux_cell,
                                                          n_peaks, n_gaps,
                                                          pop_size, n_iters) for i1 in range(len(color_XYZ)))

                    # for i1 in range(len(color_XYZ)):
                    #     eff_result_par = internal_run.run(color_XYZ[i1], color_names[i1],
                    #                                           col_thresh, photon_flux_cell,
                    #                                           n_peaks, n_gaps,
                    #                                           pop_size, n_iters, n_trials=n_trials)
