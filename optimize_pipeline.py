...
class Optimize:
    def __init__(self, colors_tuple, color_name):
        ...
        self.result1d_Minimize = minimize(self.op_delta_E_1dip,self.x0,method= method1, bounds=self.ranges,tol = tolerance)#,options={'return_all': True,'fatol': 0.1,'adaptive': True})#'initial_tr_radius': 0.5})
        self.result1gauss_Minimize = minimize(self.op_delta_E_1gauss,self.x0,method=method1, bounds=self.ranges,tol = tolerance)
        self.result2d_Minimize = minimize(self.op_delta_E_2dip,self.x02d,method=method1, bounds=self.ranges2d,tol = tolerance)
        self.result2gauss_Minimize = minimize(self.op_delta_E_2gauss,self.x02d,method=method1, bounds=self.ranges2d,tol = tolerance)
