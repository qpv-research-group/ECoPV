...
class SQlim(object):
    print(object)
    def __init__(self,spectrum,Eg, figure_title="", save_figure=True , T=300, EQE_EL=1.0, intensity=1.0):
        """
        T: temperature in K
        EQE_EL: radiative efficiency (EL quantum yield)
        intensity: light concentration, 1.0 = one Sun, 100 mW/cm^2
        cd is color data, center, width, peak, spectrum generated from op_send
        """
        try:
        ...
        ref_solar = pd.read_csv("ASTMG173.csv", header=1)  # nm vs W m^-2 nm^-1 #white
        
        self.k = 1.38064852e-23  # m^2 kg s^-2 K^-1, Boltzmi constant
        self.h = 6.62607004e-34   # m^2 kg s^-1    , planck constant
        self.c = 2.99792458e8     # m s^-1         , speed of light
        self.eV = 1.6021766208e-19  # joule        , eV to joule
        self.q = 1.6021766208e-19  # C             , elemental charge
        
       
        self.WL,  self.solar_per_nm = ref_solar.iloc[:, 0], ref_solar.iloc[:, 2]
        E = 1240.0 /  self.WL  # eV
        # jacobian transformation, W m^-2 eV^-1
        solar_per_E =  self.solar_per_nm * (self.eV/1e-9) * self.h * self.c / (self.eV*E)**2
        
        wl_color_spec = np.arange(380,780.02,0.02)        
        
        intfunc = interp1d(wl_color_spec,spectrum,fill_value="extrapolate")
        self.spectrum_int = intfunc(self.WL) 
        # visible range is WL[200:621], 380 to 780 nm, the rest should be 1, setting boundaries to be from 360 nm to 800 nm
        self.spectrum_int[0:160] = 1 
        self.spectrum_int[641:-1] = 1
        
        self.solar_per_nm_colored =  self.solar_per_nm*self.spectrum_int#[0]
        self.solar_per_E_colored = solar_per_E*self.spectrum_int#[0] # not sure if these need to be changed or no
        
        self.T = T
        self.EQE_EL = EQE_EL
        self.intensity = intensity
        self.Es = np.arange(0.32, 4.401, 0.002)
        #self.Es = Es  # np.arange(0.32, 4.401, 0.002)
        self.calculate()
        self.WLs = np.arange(280, 4001, 1.0)
        self.AM15nm = np.interp(self.WLs,  self.WL, self.solar_per_nm)
        self.AM15nm_colored = np.interp(self.WLs,  self.WL,  self.solar_per_nm_colored)
        
        # linear interpolation to get an equally spaced spectrum
        AM15 = np.interp(self.Es, E[::-1], solar_per_E[::-1])  # W m^-2 eV^-1
        AM15flux = AM15 / (self.Es*self.eV)  # number of photon m^-2 eV^-1 s^-1
        
        self.AM15_colored = np.interp(self.Es, E[::-1], self.solar_per_E_colored[::-1])  # W m^-2 eV^-1
        self.AM15flux_colored = self.AM15_colored / (self.Es*self.eV)  # number of photon m^-2 eV^-1 s^-1
    ...
    def calculate(self):
        ...
        self.Jsc = self.__cal_E_Jsc()  # short-circuit current density
        self.Voc = self.__cal_E_Voc()  # open circuit voltage
        self.PCE = self.__cal_E_PCE()  # power conversion efficiency
        self.FF = self.PCE / (self.Voc*self.Jsc) * 100.0 * self.intensity        # fill factor (%)
    ...
    def __cal_E_Jsc(self):
        ...
        #ADDED THESE LINES FROM INIT AGAIN, FOR SOME REASON COULDNT GET CLASS SELF VALUES OTHERWISE
        ref_solar = pd.read_csv("ASTMG173.csv", header=1)  # nm vs W m^-2 nm^-1 #white
        WL, solar_per_nm = ref_solar.iloc[:, 0], ref_solar.iloc[:, 2]
        E = 1240.0 / WL
        solar_per_E = solar_per_nm * (self.eV/1e-9) * self.h * self.c / (self.eV*E)**2
        solar_per_nm_colored = solar_per_nm*self.spectrum_int
        solar_per_E = solar_per_nm * (self.eV/1e-9) * self.h * self.c / (self.eV*E)**2
        solar_per_E_colored = solar_per_E*self.spectrum_int
       
        AM15_colored = np.interp(self.Es, E[::-1],solar_per_E_colored[::-1])  # W m^-2 eV^-1
        AM15flux_colored = AM15_colored / (self.Es*self.eV)        
        fluxcumm = cumtrapz(AM15flux_colored[::-1], self.Es[::-1], initial=0)
        fluxaboveE = fluxcumm[::-1] * -1 * self.intensity
        Jsc = fluxaboveE * self.q * 0.1  # mA/cm^2  (0.1: from A/m2 to mA/cm2)
        return Jsc
    ...
    def __cal_E_J0(self):
        ...
        '''
        phi = 2 * np.pi * (((self.Es*self.eV)**2) * self.eV / ((self.h**3) * (self.c**2)) / (
                           np.exp(self.Es*self.eV / (self.k*self.T)) - 1))
        ...
        J0 = fluxaboveE * self.q * 0.1 / self.EQE_EL  # (0.1: from A/m2 to mA/cm2)
    ...
    def __cal_E_Voc(self):
        ...
        '''
        return (self.k*self.T/self.q) * np.log((self.Jsc/self.J0) + 1)
    ...
    def __cal_E_PCE(self):
        ...
        for i, E in enumerate(self.Es):
            ...
            J = -1 * self.Jsc[i] + self.J0[i] * (
                np.exp(self.q*V / (self.k*self.T)) - 1)
    ...
    def simulate_JV(self, Eg, plot_dark=False, Vstep=0.001,
        ...
        J = -1.0 * Jsc + J0 * (np.exp(self.q*V / (self.k*self.T)) - 1)
        if plot_dark:
            Jdark = J + Jsc
        mask = (J <= 200)
    ...
    def E_loss(self, Eg, xmin=300, xmax=2500, savefig=False):
        ...
        WLs = self.WLs
        AM15nm = self.AM15nm
        AM15nm_colored = self.AM15nm_colored
        if self.save_figure:
            ...
            extract = AM15nm_colored / (1240.0/WLs) * Eg * mask
            Eavail = extract * factor
            therm = AM15nm_colored * mask
    
            extractloss = extract - Eavail
            thermloss = therm - extract
            transloss = AM15nm * (1 - mask)
            colorloss = AM15nm - AM15nm_colored
    
            ax.fill_between(WLs, 0, AM15nm,
                            linewidth=0, facecolor=colors['color'])
            ...
            labels = ["{:.1f}% Not Absorbed".format(100.0 * E_pct['trans']),
                      ...
                      "{:.1f}% Reflected Energy".format(100.0 * E_pct['color'])]

        if self.save_figure:
            ax.plot([Eg], [0])
            ax.legend(legends, labels, frameon=False, title=legendtitle,
                      fontsize=14, loc="upper right").set_draggable(state=True)
        ...
        losses["Reflected Energy"] = colorloss

        return losses

    def available_E(self, Egs, E_MPP=True, xmin=300, xmax=2500,
                    savefig=False, legend=True, legend_totE=True, legend_ColorLoss=True):
        """
        ...
        WLs = self.WLs
        AM15nm = np.interp(self.WLs, self.WL, self.solar_per_nm)
        AM15nm_colored = np.interp(self.WLs, self.WL,  self.solar_per_nm_colored)
        
        if self.save_figure:
            plt.figure(figsize=(8, 4.5))
            ax = plt.gca()
            # color options: darkgoldenrod, darkorange, yellow, black
            ax.fill_between(WLs, 0, AM15nm, linewidth=0.0, facecolor="black")
            ax.fill_between(WLs, 0, AM15nm_colored, linewidth=0.0, facecolor="lightGrey")
        ...
        E_subcell_CM=E_subcell
        PCEsubcell, PCEsubcell_CM, tot_E = [], [], 1000.0
        ...
        for n, Eg in enumerate(Egs[:-1]):
            color = colors[n]
            if E_MPP:
                SQ_E = SQlim(self.spectrum,Eg,self.figure_title,intensity=(Jscs[n]-Jscs[n+1]) / Jscs[n],
                             T=self.T, EQE_EL=self.EQE_EL)
                para = SQ_E.get_paras(Eg, toPrint=False)
                factor = para["Voc"] * para["FF"] / 100.0 / Eg
            mask = ((1240.0/WLs) >= Eg) * (Egs[n+1] >= (1240.0/WLs))
            Eavail = factor * AM15nm_colored / (1240.0/WLs) * Eg * mask
            ...
            J_subcell = AM15nm_colored / (1240.0/WLs) * Eg * mask # added by Farid on 26.8.2021, current matching, from here:
            if np.sum(J_subcell) < np.sum(J_min):
                J_min = J_subcell
        if numEg != 1:
            for n, Eg in enumerate(Egs[:-1]):
                color = colors[n]
                dark_factor=0.5
                color_dark=[float(color[0])*dark_factor,float(color[1])*dark_factor,float(color[2])*dark_factor,float(color[3])*dark_factor]
                color=tuple(color_dark)
                para = SQ_E.get_paras(Eg, toPrint=False)
                factor = para["Voc"] * para["FF"] / 100.0 / Eg           
                mask = ((1240.0/WLs) >= Eg) * (Egs[n+1] >= (1240.0/WLs))
                Eavail = factor * AM15nm_colored / (1240.0/WLs) * Eg * mask
                Eavail_CM = factor * J_min
                
                CM_drop_factor=np.sum(Eavail_CM)/np.sum(Eavail)
    
                E_subcell_CM["Eg=" + str(round(Eg, 3))] = Eavail_CM
                PCEsubcell_CM.append(100 * np.sum(Eavail_CM) / tot_E)
                if self.save_figure:                
                    ax.fill_between(WLs, 0, Eavail*CM_drop_factor, facecolor=color, linewidth=0.0)
        else:
            PCEsubcell_CM=PCEsubcell
        ...
        # Up to here
            ...
            ax.set_xlim(xmin, xmax)
            ax.set_ylabel("Irradiance  (W $\mathregular{m^{-2}\ nm^{-1}}$)",
                          size=16, fontname="Arial")
            ax.set_xlabel("Wavelength (nm)", size=16, fontname="Arial")
    ...
    #        ax.grid(False)
        ...
        if legend:
            ...
            legends.append(plt.Rectangle((0, 0), 1, 1, facecolor="black"))

            if numEg == 1:
                labels = ["E$_g$={0} eV, $\eta$ = {1:.1f}%, $\eta$$_c$$_m$ = {2:.1f}%".format(Egs[0], PCEsubcell[0], PCEsubcell_CM[0])]
                eff_final=PCEsubcell[0]
                eff_final_CM=PCEsubcell_CM[0]
                labels.append("Reflected Energy={:.1f}%".format(100 - 100.0 * np.sum(AM15nm_colored)/np.sum(AM15nm)))

            else:
                labels = ["Cell {0}, E$_g$={1:.2f} eV, $\eta_{0}$={2:.1f}%, $\eta_{0}$$_c$$_m$={3:.1f}%".format(
                          numEg - i, Egs[i], PCEsubcell[i], PCEsubcell_CM[i])
                          for i in range(numEg - 1, -1, -1)]
                labels.append("Reflected Energy={:.1f}%".format(100 - 100.0 * np.sum(AM15nm_colored)/np.sum(AM15nm)))
            if self.save_figure:
                ax.legend(legends, labels, frameon=False, loc="upper right",
                          fontsize=12).set_draggable(state=True)

        if numEg != 1 and legend_totE:
            totE = np.nansum(PCEsubcell)  # small slice would have nan PCE
            eff_final=totE
            totE_CM = np.nansum(PCEsubcell_CM)  # small slice would have nan PCE
            eff_final_CM=totE_CM
            legend_title = "Total $\eta$ = {0:.1f}%, Total $\eta$$_c$$_m$ = {1:.1f}%".format(totE, totE_CM)
            legends = [plt.Rectangle((0, 0), 1, 1, facecolor="black"),
                       plt.Rectangle((0, 0), 1, 1, facecolor="Green")]
            labels=["Reflected Energy={:.1f}%".format(100 - 100.0 * np.sum(AM15nm_colored)/np.sum(AM15nm))]
            if self.save_figure:
                if not ax.get_legend():
                    ax.legend(legends, labels, frameon=False, loc="upper right",
                              title=legend_title, fontsize=14).set_draggable(state=True)
                else:
        ...
        if self.save_figure:
            ...
            plt.tight_layout()
            cwd = os.getcwd()
            filepath=os.path.join(cwd, r"plots",self.figure_title)
        ...
        if numEg == 1:
                ...
                eff_final=totE
                totE_CM = np.nansum(PCEsubcell_CM)
                eff_final_CM=totE_CM
        print('EFFFICIENCY FINAL IS ',eff_final)
        print('EFFFICIENCY CURRENT MATCHED IS ',eff_final_CM)
        self.eff_final = eff_final
        self.eff_final_CM = eff_final_CM

        return E_subcell, PCEsubcell, eff_final, eff_final_CM
    ...
    def __helper_plt(cls, xlim, attr="PCE"):
        ...
        ax = plt.gca()
        ax.legend(loc='upper right', fontsize=16).set_draggable(state=True)
        plt.tight_layout()
...
#    SQ.plotall()
    # SQ.E_loss(1.125)
    # SQ.E_loss(1.9)
    # SQ.available_E(1.125,2.1) # Eg of PSC is 1.5 - 2.2
#

#   Double-junction solar cells using Tandem Si-PSC
    # SQ.available_E(Egs=[1.125, 1.750]) #Opt white, eff=44.63(python2)/45.1(python1)
#    SQ.available_E(Egs=[1.125, 1.725]) #Opt blue, eff=40.38/38.5
#    SQ.available_E(Egs=[1.125, 2.1]) #Opt red, eff=36.14/36.7
#    SQ.available_E(Egs=[1.125, 1.7250]) #Opt green, eff=43.49/42.4
#    SQ.available_E(Egs=[1.125, 1.7250]) #Opt grey, eff=39.88/38.7

#   Triple-junction solar cells GaInP/GaAs/GaInAs
    # SQ.available_E(Egs=[1, 1.424, 1.9]) #Eff: white(AM1.5G)=50.5, red=38.7, green=47.6, blue= 43.2, grey=43.8

    # SQ.available_E(np.linspace(0.32, 4.2, 75), legend=False) # 75-junction

    # plt.show()


#plt.plot(color[0,:],1-color[1,:]) # plot the color spectrum
