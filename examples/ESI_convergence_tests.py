...
# col_thresh and efficiency change between batches is less than this.
...
if __name__ == "__main__":
    ...
    for j1, n_junctions in enumerate(n_junc_loop):
        ...
        for i1, n_peaks in enumerate(n_peak_loop):
            ...
            # plt.show()
                ...
                #         ax_list[m1].plot(final_pop[m1][l1].get_population().get_f()[:, 0], final_pop[m1][l1].get_population().get_f()[:, 1], 'x')
                # plt.show()
    ...
    fig, axs = plt.subplots(len(n_junc_loop), len(color_names), figsize=(10, 1.7*len(n_junc_loop)))
    ...
    eff_array_sorted = np.sort(eff_array, axis=2)

    eff_array_trunc = eff_array_sorted[:,:, 1:-1,:]

    confidence_interval = np.max(eff_array_trunc, axis=2) - np.min(eff_array_trunc, axis=2)
    ...
    for j1, n_junctions in enumerate(n_junc_loop):
        ...
        for l1 in range(len(color_names)):
            ...
            max_count = np.max([np.max(np.histogram(eff_array[j1, 0, :, l1],
                                                    bins=bins
                                                    )[0])])# for m1 in range(len(n_peak_loop))])
