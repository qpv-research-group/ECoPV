...
max_trials_col = 4*add_iters  # how many population evolutions happen before giving up if there are no populations
...
# Need this __main__ construction because otherwise the parallel running of the different islands (n_trials) may throw an error
    ...
    for i1, Y in enumerate(Ys):
        ...
        if os.path.exists(save_path + "/pop_gamut_Y_{}_{}_spd_2.npy".format(Y, n_junctions)):
            print("Load existing result")

            pop_array = np.load(save_path + "/pop_gamut_Y_{}_{}_spd_2.npy".format(Y, n_junctions))
            eff_array = np.load(save_path + "/eff_gamut_Y_{}_{}spd_2.npy".format(Y, n_junctions))

        else:
            col_possible_str = save_path + "/possible_colours_Y_{}_spd_2.txt".format(Y)
            print("Found existing file for possible colours")

            if os.path.exists(col_possible_str):
                ...
                if i1 == 0:
                    ...
                    col_possible_str_pr = save_path + "/possible_colours_Y_{}_spd_2.txt".format(Ys[i1 - 1])
        ...
        # for i1, n_junctions in enumerate(n_junc_loop):
            ...
            np.save(save_path + "/pop_gamut_Y_{}_{}_spd_2.npy".format(Y, n_junctions), pop_array)
            np.save(save_path + "/eff_gamut_Y_{}_{}_spd_2.npy".format(Y, n_junctions), eff_array)
