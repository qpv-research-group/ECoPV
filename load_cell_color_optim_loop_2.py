    ...
    for i1, n_peaks in enumerate(n_peak_loop):
        for k1, fixed_height in enumerate(fixed_height_loop):
            ...
            if i1 == len(n_peak_loop) - 1:
                ax1.plot(eff_xr.color.data,
                         eff_xr.data, mfc='none',
                         linestyle='none',
                ...
                ax1.plot(eff_xr.color.data,
                         eff_xr.data, mfc='none',
                         linestyle='none',
                         color=cols[i1], marker=shapes[j1], alpha=alphas[k1]
                         )

            eff_loss = 100 * (eff_xr - black_cell_eff[n_junctions - 1]) / black_cell_eff[n_junctions - 1]

            if j1 == len(n_junc_loop) - 1:
                ax2.plot(eff_loss.color.data, eff_loss.data,
...
for n_junctions in n_junc_loop:
    ...
    black_cell_eff_bb.append(100*getPmax(Egs, light_source.spectrum(wl_cell)[1], light_source.spectrum(wl_cell)[0], interval)/light_source.power_density)
...
for j1, n_junctions in enumerate(n_junc_loop):
    for i1, n_peaks in enumerate(n_peak_loop):
        for k1, fixed_height in enumerate(fixed_height_loop):
            ...
            champion_effs = np.loadtxt("results/champion_eff_" + type + str(n_peaks) + '_' + str(
                n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base) + 'bb_spd.txt')
            champion_pops = np.loadtxt("results/champion_pop_" + type + str(n_peaks) + '_' + str(
                n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base) + 'bb_spd.txt')
            ...
            eff_loss = 100 * (eff_xr - black_cell_eff_bb[n_junctions - 1]) / black_cell_eff_bb[n_junctions - 1]
...
for j1, n_junctions in enumerate(n_junc_loop):
    for i1, n_peaks in enumerate(n_peak_loop):
        for k1, fixed_height in enumerate(fixed_height_loop):
            ...
            axs[j1].plot(Eg_xr.color.data, 1240 / Eg_xr.data, mfc='none',
                         linestyle='--',
                         # color=cols[i1], marker=shapes[j1],
                         marker='o',
                         markersize=4,
...
for j1, n_junctions in enumerate(n_junc_loop):
    ...
    print(n_junctions, np.max(champion_eff_comp[1] - champion_eff_comp[0]))  # if positive, variable height is better;
    ...
    for i1, n_peaks in enumerate(n_peak_loop):
        for k1, fixed_height in enumerate(fixed_height_loop):
            ...
            for l1, target in enumerate(color_XYZ_xr):
                ...
                axes[0, 0].plot(l1 + offset[offset_ind] - data_width / 2, eff_xr.data[l1], '.',
                                color=colors[offset_ind],
                                markersize=4)

                axes[1, 0].errorbar([l1 + offset[offset_ind] - data_width / 2] * len(centres), centres, yerr=widths / 2,
                                    fmt='none',
...
for i1 in range(len(RGBA)):
    axes[1, 1].add_patch(
        Rectangle(xy=(0, photon_flux_color[0][i1]),
                  width=photon_flux_color[1][i1] / np.max(photon_flux_color),
                  height=interval,
                  facecolor=RGBA[i1, :3], alpha=0.6 * RGBA[i1, 3])
    )
    ...
    for i1, n_peaks in enumerate(n_peak_loop):
        for k1, fixed_height in enumerate(fixed_height_loop):
            ...
            for l1, target in enumerate(color_XYZ_xr):
                ...
                axes[1, 0].plot(l1 + offset[offset_ind] - data_width / 2, 1240 / Eg_xr.data[l1, -1], 'o',
                                mfc='none',
                                markersize=3,
                                color=colors[offset_ind])
...
for j1, n_junctions in enumerate(n_junc_loop):
    ...
    w_xr = make_sorted_xr(champion_pops[:, n_peaks:2 * n_peaks], color_names)[subset_inds]
    ...
    for l1, target in enumerate(color_XYZ_subs):
        ...
        axes[0].plot(l1 + offset[offset_ind] - data_width / 2, eff_xr.data[l1], '.',
                        color=colors[offset_ind],
                        markersize=4)

        axes[1].errorbar([l1 + offset[offset_ind] - data_width / 2] * len(centres), centres, yerr=widths / 2,
                            fmt='none',
                            ecolor=colors[offset_ind])

        axes[1].plot(l1 + offset[offset_ind] - data_width / 2, 1240 / Eg_xr.data[l1, -1], 'o',
                        mfc='none',
                        markersize=3,
                        color=colors[offset_ind])
...
photon_flux_eV = LightSource(
    source_type="standard", version="AM1.5g", x=1240 / wl_cell, output_units="photon_flux_per_ev"
)

champion_pops = np.loadtxt("results/champion_pop_" + type + str(2) + '_' + str(
    1) + '_' + "True" + str(1) + '_' + str(0) + '.txt')

white_ch_pop = champion_pops[-6]
spec_white = gen_spectrum_ndip(white_ch_pop, 2, photon_flux_cell[0], max_height=1, base=0)
...
for i1, Eg in enumerate(1240 / wl_cell):
    Vmax[i1], Imax[i1] = getIVmax([Eg], photon_flux_cell[1], photon_flux_cell[0], interval)
    Vmax_w[i1], Imax_w[i1] = getIVmax([Eg], (1 - spec_white) * photon_flux_cell[1], photon_flux_cell[0], interval)
