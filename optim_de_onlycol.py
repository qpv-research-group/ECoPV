import numpy as np
from Spectrum_Functions_de import spec_to_xyz, delta_E_CIE2000, gen_spectrum_1dip, \
    gen_spectrum_2dip, gen_spectrum_1gauss, gen_spectrum_2gauss, delta_XYZ
...
class Optimize:
    def __init__(self, colors_tuple, color_name):
        ...
        self.target_color = np.array(self.xyz.get_value_tuple())

        self.dE_threshold1d = 0.25
        self.dE_threshold = 0.25
        ...
        result2d_Minimize = PDE_obj_2dip.solve()
        res_color = result2d_Minimize
        best_pop_2d = res_color[0]

        if best_pop_2d[0] > best_pop_2d[2]:
            # switch peaks for consistency
            pair1 = best_pop_2d[:2]
            pair2 = best_pop_2d[2:4]
            best_pop_2d = np.hstack((pair2, pair1))

        self.params = best_pop_2d

        self.final_dE = color_2d.calculate(best_pop_2d)[0]
        self.final_n = res_color[1]


        # print('final fitness', res_color[1])
        #
        # if res_color[1] < 0:
        #     print("Met colour threshold")

        # best_pop_evo = res_color[2]
        best_fitn_evo = res_color[3]
        mean_fitn_evo = res_color[4]
        # final_fitness = res[1]
...
#         plt.show()
        ...
        #
        # palette = np.array([[rgb_target_list, rgb_d2_list]], dtype=int)
        #
        # io.imshow(palette)
        # plt.show()
...
class eff_colour_2dip():
    ...
    def evaluate(self,x):
        ...
        if delta < self.threshold: # meets colour threshold; now try to optimize how efficiently we can make this colour
