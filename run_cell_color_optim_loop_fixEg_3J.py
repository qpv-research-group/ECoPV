...
for n_junctions in n_junc_loop:
    ...
    if n_junctions > 0:
        ...
        save_loc = save_path + "/champion_pop_{}juncs_{}spec_3J.txt".format(n_junctions, light_source_name)
...
# Need this __main__ construction because otherwise the parallel running of the different islands (n_trials) may throw an error
    ...
    for n_peaks in n_peak_loop:
        for i1, n_junctions in enumerate(n_junc_loop):
            ...
            if n_junctions > 0:
                Eg_guess = np.loadtxt(save_path + "/champion_pop_{}juncs_{}spec_3J.txt".format(n_junctions, light_source_name),
                                      ndmin=1)
            ...
            for fixed_height in fixed_height_loop:
                ...
                save_name = "results/champion_eff_" + type + str(n_peaks) + '_' + str(
                    n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base)+'_3J_spd.txt'
                ...
                if not path.exists(save_name):
                    ...
                    np.savetxt("results/champion_eff_" + type + str(n_peaks) + '_' + str(
                        n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base)+'_3J_spd.txt', champion_effs)
                    np.savetxt("results/champion_pop_" + type + str(n_peaks) + '_' + str(
                        n_junctions) + '_' + str(fixed_height) + str(max_height) + '_' + str(base) +'_3J_spd.txt', champion_pops)
    ...
    J2_c[-1] = Is[1]
    J3_c[-1] = Is[2]
    ax2.plot(eff_xr.color, J1_c/10, label="InGaP", color=pal[0], marker=shapes[1], alpha=0.5, linestyle='--')
    ax2.plot(eff_xr.color, J2_c/10, label="GaAs", color=pal[0], marker=shapes[2], alpha=0.5, linestyle='--')
    ax2.plot(eff_xr.color, J3_c/10, label="Ge", color=pal[0], marker='s', alpha=0.5, linestyle='--')
    ...
    ax.set_xticklabels(eff_xr.color.data, rotation=45, ha='right', rotation_mode='anchor')
    ax.set_ylabel("Efficiency (%)")
    ax.arrow(4.5, 41, -4.1, 0, head_width=0.7, head_length=0.5,
             overhang=0.2, color='k', alpha=0.6)
    ax2.arrow(22, 15, 1.75, 0, head_width=0.7, head_length=0.5,
              overhang=0.2, color=pal[0], alpha=0.6)
    ax2.set_ylabel(r"$J_{max}$ per junction")
    ax2.legend(loc=(0.8, 0.1))
    ax.set_ylim(12, 44)
    ax2.set_ylim(0, 35)
    apply_formatting(ax, color_labels=eff_xr.color.data, n_colors=len(eff_xr.color.data))
