...
max_trials_col = 2*add_iters  # how many population evolutions happen before giving up if there are no populations
...
# Need this __main__ construction because otherwise the parallel running of the different islands (n_trials) may throw an error
    ...
    for i1, Y in enumerate(Ys):
        ...
        if os.path.exists(save_path + "/pop_gamut_Y_{}_{}.npy".format(Y, n_junctions)):
            print("Load existing result")

            pop_array = np.load(save_path + "/pop_gamut_Y_{}_{}.npy".format(Y, n_junctions))
            eff_array = np.load(save_path + "/eff_gamut_Y_{}_{}.npy".format(Y, n_junctions))
            ...
            if os.path.exists(save_path + "/possible_colours_Y_{}.txt".format(Y)):
                ...
                if i1 == 0:
                    ...
                    col_possible_str_pr = save_path + "/possible_colours_Y_{}.txt".format(Ys[i1 - 1])
        ...
        # for i1, n_junctions in enumerate(n_junc_loop):
            ...
            np.save(save_path + "/pop_gamut_Y_{}_{}.npy".format(Y, n_junctions), pop_array)
            np.save(save_path + "/eff_gamut_Y_{}_{}.npy".format(Y, n_junctions), eff_array)
        ...
        levels = np.arange(np.ceil(np.min(eff_array[eff_array>0]))+1, 1.01*np.floor(np.nanmax(eff_array[eff_array>0])), 1)
